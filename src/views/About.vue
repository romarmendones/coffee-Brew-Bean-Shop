  ...
  >
    <div class="relative w-full max-w-3xl bg-black rounded-2xl overflow-hidden shadow-2xl">
      <button
        ...
        class="absolute top-3 right-3 z-10 p-2 bg-white/20 hover:bg-white/40 rounded-full transition-colors text-white"
      >
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
        </svg>
      ...
      <div class="aspect-video">
        <iframe
          class="w-full h-full"
          src="https://www.youtube.com/embed/d_IFKP1Ofq0?autoplay=1"
      ...
      <div class="px-6 py-4 bg-coffee-900 text-cream-100">
        <p class="font-semibold text-lg">Brew & Bean — Our Story</p>
        <p class="text-sm text-cream-300">From humble beginnings to the heart of the community.</p>
      </div>
    </div>
  </div>
  ...
  >
    <div class="w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden">
      <div class="flex items-center justify-between px-6 py-5 bg-gradient-to-r from-coffee-700 to-coffee-900 text-white">
        <h3 class="text-xl font-bold font-serif">{{ selectedValue.title }}</h3>
        <button @click="closeValueModal" class="p-1 rounded-lg hover:bg-white/20 transition-colors">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
          </svg>
        </button>
      ...
      <div class="px-6 py-6">
        <p class="text-coffee-600 leading-relaxed mb-6">{{ selectedValue.details }}</p>
        <div class="flex justify-end">
          <button @click="closeValueModal" class="btn-primary px-6 py-2">Got it!</button>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden">
      <div class="flex items-center justify-between px-6 py-5 bg-gradient-to-r from-coffee-700 to-coffee-900 text-white">
        <h3 class="text-xl font-bold font-serif">
          <span v-if="!tourSubmitted">Schedule a Coffee Tour</span>
          <span v-else>Tour Booked! ☕</span>
        </h3>
        <button @click="closeTourModal" class="p-1 rounded-lg hover:bg-white/20 transition-colors">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
          </svg>
        </button>
      </div>
      <div v-if="!tourSubmitted" class="px-6 py-6">
        <p class="text-coffee-600 mb-5">Experience our roasting facility, meet our team, and taste exceptional coffee — all for free!</p>
        <form @submit.prevent="submitTour" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block mb-1 text-sm font-medium text-coffee-700">Name</label>
              <input v-model="tour.name" type="text" required placeholder="Your full name"
            ...
            <div>
              <label class="block mb-1 text-sm font-medium text-coffee-700">Phone</label>
              <input v-model="tour.phone" type="tel" required placeholder="09XXXXXXXXX"
                class="w-full px-3 py-2 border border-coffee-200 rounded-lg focus:ring-2 focus:ring-coffee-500 focus:border-transparent text-sm" />
            </div>
          </div>
          <div>
            <label class="block mb-1 text-sm font-medium text-coffee-700">Email</label>
            <input v-model="tour.email" type="email" required placeholder="your@email.com"
              class="w-full px-3 py-2 border border-coffee-200 rounded-lg focus:ring-2 focus:ring-coffee-500 focus:border-transparent text-sm" />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block mb-1 text-sm font-medium text-coffee-700">Preferred Date</label>
              <input v-model="tour.date" type="date" required :min="minDate"
                class="w-full px-3 py-2 border border-coffee-200 rounded-lg focus:ring-2 focus:ring-coffee-500 focus:border-transparent text-sm" />
            </div>
            <div>
              <label class="block mb-1 text-sm font-medium text-coffee-700">Time Slot</label>
              <select v-model="tour.time" required class="w-full px-3 py-2 border border-coffee-200 rounded-lg focus:ring-2 focus:ring-coffee-500 focus:border-transparent text-sm">
          ...
          <div>
            <label class="block mb-1 text-sm font-medium text-coffee-700">Group Size</label>
            <select v-model="tour.groupSize" class="w-full px-3 py-2 border border-coffee-200 rounded-lg focus:ring-2 focus:ring-coffee-500 focus:border-transparent text-sm">
          ...
          <button type="submit" class="w-full btn-primary py-3 mt-2">Confirm Tour Booking</button>
        </form>
      </div>
      <div v-else class="px-6 py-8 text-center">
        ...
        <h4 class="mb-2 text-xl font-bold text-coffee-800">You're all set, {{ tour.name }}!</h4>
        <p class="text-coffee-600 mb-1">Your tour is scheduled for <strong>{{ tour.date }}</strong> at <strong>{{ tour.time }}</strong>.</p>
        <p class="text-sm text-coffee-500 mb-6">We'll send a confirmation to <strong>{{ tour.email }}</strong>.</p>
        <button @click="closeTourModal" class="btn-primary px-8 py-3">Great, Thanks!</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    // Download PDF (generates a simple text blob as demo)
    const downloadStoryPDF = () => {
      const content = `BREW & BEAN — OUR STORY\n\nFounded in 2010 by Sarah Chen, Brew & Bean started as a small coffee cart and grew into a beloved community gathering place.\n\nOUR MISSION\nTo craft exceptional coffee experiences that bring people together.\n\nOUR VALUES\n• Quality First — We never compromise on quality\n• Sustainability — Ethical sourcing and eco-friendly practices\n• Community — Building strong relationships\n\nCONTACT\nbrew.and.bean@gmail.com\n09123456789\nMakati, Metro Manila`
      const blob = new Blob([content], { type: 'text/plain' })
      const url = URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = url
      link.download = 'Brew-and-Bean-Our-Story.txt'
      link.click()
    ...
    const closeTourModal = () => {
      ...
      tourSubmitted.value = false
      tour.value = { name: '', phone: '', email: '', date: '', time: '', groupSize: '1' }
    }
